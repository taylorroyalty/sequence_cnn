...
#Sampling Parameters
rep=3
test_val_sample=range(2,30,2) #start on 18
# sample_rate=0.98
...
df_fit=pd.DataFrame()
for s in test_val_sample:
    train_sample=round((s-0.3*s)/.3)
    for r in range(rep):
        for anno in uniq_anno:
            ...
            df_train['n_sample']=train_sample
            ...
            df_test_val['n_sample']=train_sample
...
df_fit.to_csv(train_val_save_path+'_'+dr_type+'.csv')
